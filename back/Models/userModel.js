...
export const getAll = () =>
  query(
    "SELECT u.id, u.username, u.email, u.phone, r.name AS role, u.state FROM users u JOIN role r ON u.role_id = r.id WHERE u.state = 0"
  );

export const getById = (id) =>
  query(
    "SELECT u.id, u.username, u.email, u.phone, u.password, r.name AS role, u.state FROM users u JOIN role r ON u.role_id = r.id WHERE u.id = ? AND u.state != 0",
    [id]
  );
...
//Revisar el estado
export const updatePasswordAndState = (id, password) =>
  query("UPDATE users SET password = ?, state = 2 WHERE id = ?", [password, id]);
